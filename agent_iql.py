...
class Agent():
    def __init__(self, state_size, action_size, config):
        ...
        z_dim = 32
        u_dim = 1
        self.model = E2C(obs_dim=obs_dim, z_dim=z_dim, u_dim=u_dim).to(self.device)
        self.optimizer_e2c = optim.Adam(self.model.parameters(), betas=(0.9, 0.999), eps=1e-8, lr=self.lr)
        # self.mdrnn.load_state_dict({k.strip('_l0'): v for k, v in rnn_state['state_dict'].items()}) 
        #self.optimizer = torch.optim.Adam(self.mdrnn.parameters(), lr=1e-4)   # change to adam and learning lower lr
    ...
    def learn(self, memory_ex):
        ...
        self.steps += 1
        states, actions, next_states, dones = memory_ex.sample(self.batch_size)
        states = torch.as_tensor(states, device=self.device).type(torch.float64).div_(255)
        next_states = torch.as_tensor(next_states, device=self.device).type(torch.float64).div_(255)
        x_recon, x_next_pred, q_z, q_z_next_pred, q_z_next = self.model(states, actions, next_states)
        lamda = 0.25
        #x = self.model.encoder(states)[0]
        # x_next =  self.model.encoder(next_states)[0]
        # print(x_recon.shape)
        loss = self.compute_loss(states, next_states, q_z_next, x_recon, x_next_pred, q_z, q_z_next_pred, lamda)
        # states = states.transpose(0, 3, 1, 2)
        states = self.model.encoder(states)[0]
        self.state_action_frq(states, actions)
        #next_states = next_states.transpose(0, 3, 1, 2)
        next_states = self.model.encoder(next_states)[0]
    ...
    def compute_loss(self, x, x_next, q_z_next, x_recon, x_next_pred, q_z, q_z_next_pred, lamda):
        # lower-bound loss
        ...
        loss = lower_bound + lamda * consis_term
        loss.backward()
        self.writer.add_scalar('model loss', loss, self.steps)
    ...
    def test_predicter(self, memory):
        """ Test the classifier """
        self.predicter.eval()
        same_state_prediction = 0
        test_elements = np.random.randint(0, memory.idx, size=100)
        for i in test_elements:
            ...
            states = states.type(torch.float64).div_(255)
            states = self.model.encoder(states)[0] 
            ...
            if y == p:
                same_state_prediction += 1
        text = "Same prediction {} of {} ".format(same_state_prediction, len(test_elements))
        self.writer.add_scalar('Predicter100Random', same_state_prediction, self.steps)
        print(text)
    ...
    def load(self, filename):
        ...
        # load vae and mndrnn
        self.model.load_state_dict(torch.load(filename + "_model_net.pth"))
        print("Load models  {}".format(filename))
    ...
    def test_q_value(self, memory):
        ...
        text = "Kl div of Reward {} of {} elements".format(average_r_kl, used_elements_r)
        print(text)
    ...
    def act(self, states):
        states = np.array(Image.fromarray(states).resize((64,64)))
        states = torch.as_tensor(states.transpose(2, 0, 1), device=self.device).unsqueeze(0)
        states = states.type(torch.float64).div_(255) 
        states = self.model.encoder(states)[0]
        q_values = self.qnetwork_local(states.detach()).detach()
    ...
    def eval_policy(self, record=False, random_policy=False, eval_episodes=2, eval_policy=False, steps=0):
        ...
        for i_epiosde in range(eval_episodes):
            ...
            while True:
                ...
                cv2.imwrite("{}/{}/pic-{}.png".format("vid", i_epiosde, s), np.array(state))
        ...
        if random_policy:
            ...
            print(" Random policy Eval Episode {}  average Reward {} ".format(eval_episodes, average_reward))
